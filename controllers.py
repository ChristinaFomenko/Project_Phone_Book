...
def find_user_controller(data=None, cls=True) :
    render_template(context={}, template="find_user.jinja2", cls=cls)
    username = input()
    user = User.add(username)
    return 11, username


def show_number_controller(username, cls=True) :
    render_template(context={}, template="show_number.jinja2", cls=cls)
    user = User.add(username)
    return 12, user
...
controllers_dict = {
    ...
    '3' : find_user_controller,
    21 : add_phone_controller,
    212 : add_more_controller,
    11 : show_number_controller,
    12 : find_user_controller
}
